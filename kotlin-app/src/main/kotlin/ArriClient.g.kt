...
class ArriClient(
    ...
    suspend fun sayHello(params: SayHelloParams): SayHelloResponse {
        ...
        throw ArriClientError.fromJson(response.bodyAsText())
    }
}



interface ArriClientModel {
    fun toJson(): String
    fun toUrlQueryParams(): String
}

interface ArriClientModelFactory<T> {
    fun new(): T
...
data class ArriClientError(
    val code: Int,
    val errorMessage: String,
    val data: JsonElement?,
    val stack: List<String>?,
) : Exception(errorMessage), ArriClientModel {
    ...
    companion object Factory : ArriClientModelFactory<ArriClientError> {
        override fun new(): ArriClientError {
            return ArriClientError(
                code = 0,
                errorMessage = "",
                data = null,
                stack = null
        ...
        override fun fromJson(input: String): ArriClientError {
            return fromJsonElement(JsonInstance.parseToJsonElement(input))
        }

        override fun fromJsonElement(__input: JsonElement, instancePath: String): ArriClientError {
            if (__input !is JsonObject) {
                __logError("[WARNING] ArriClientError.fromJsonElement() expected JsonObject at $instancePath. Got ${__input.javaClass}. Initializing empty ArriClientError.")
            }
...
return queryParts.joinToString("&")
    ...
    companion object Factory : ArriClientModelFactory<SayHelloParams> {
        @JvmStatic
...
return queryParts.joinToString("&")
    ...
    companion object Factory : ArriClientModelFactory<SayHelloResponse> {
        @JvmStatic
            ...
            if (httpResponse.status.value !in 200..299) {
                try {
                    if (httpResponse.headers["Content-Type"] == "application/json") {
                        onConnectionError(
                            ArriClientError.fromJson(httpResponse.bodyAsText())
                        )
                    } else {
                        onConnectionError(
                            ArriClientError(
            ...
            while (!channel.isClosedForRead) {
                ...
                for (event in parsedResult.events) {
                    ...
                    when (event.event) {
                        ...
                        "error" -> {
                            val error = ArriClientError.fromJson(event.data)
                            try {
                                onError(error)
                            } catch (e: CancellationException) {
                                onClose()
